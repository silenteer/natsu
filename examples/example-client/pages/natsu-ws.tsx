...
import { NatsuProvider, useDefferedRequest, useRequest, useSubscribe } from '@silenteer/natsu-react';
...
function Socket({ name }: { name: string }) {
  ...
  const sendHello = useDefferedRequest<NatsHelloNamespace>('api.helloNamespace');
  const subscriber = useSubscribe<NatsHelloNamespaceChannel>('ws.helloNamespace', async (nextMsg) => {
    setMessages((prevState) => [
      ...(prevState || []),
      nextMsg.body.message,
    ]);
  })
...
function Wrapper() {
  return <>
    <NatsuProvider natsuClient={request01} makeNatsuSocketClient={() => connectWS({
    ...
    <NatsuProvider natsuClient={request02} makeNatsuSocketClient={() => connectWS({
      serverURL: new URL('ws://localhost:8080'),
      headers: {
        'user-id': userId02,
      },
