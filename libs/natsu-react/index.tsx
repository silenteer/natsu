...
import { NatsuClient, NatsuSocketClient } from '@silenteer/natsu-port';
import type { NatsChannel, NatsPortWSResponse, NatsService } from '@silenteer/natsu-type';
...
const NatsuContext = React.createContext<NatsuContextProps | undefined>(undefined);
...
function useRequest<TService extends NatsService<string, unknown, unknown>>(
  ...
  const call = useCallback(() => natsuClient<TService>(address, data), [address, data]);

  return useAsync(call, options.immediate);
}

function useDefferedRequest<TService extends NatsService<string, unknown, unknown>>(
  address: TService['subject'],
  data?: TService['request'],
...
function useSubscribe<TChannel extends NatsChannel<string, unknown, unknown>>(
  ...
  handler: (nextMsg: NatsPortWSResponse<TChannel['subject'], TChannel['response']>) => Promise<void>,
  options: SubscribeOption = { immediate: true }
) {
  // Next will do SSR
  if (typeof window === 'undefined') {
    return { sub: noop, unsub: noop };
  }
...
function NatsuProvider({
  natsuClient, children, makeNatsuSocketClient
  ...
  return <NatsuContext.Provider value={{ natsuClient, natsuSocketClient }}>{children}</NatsuContext.Provider>
}

export { useNatsuClient, useNatsuSocketClient, NatsuProvider, useRequest, useSubscribe, useDefferedRequest };
